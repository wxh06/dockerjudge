...
def _judge(container, commands, stdio, timeout=1):
    ...
    result = container.exec_run(['sh', '-c', 'echo ' + shlex.quote(stdio[0]) +
                                 ' | timeout -k ' + (str(timeout) + ' ') * 2 +
...
def judge(settings, source='', tests=(), timeout=1, client=docker.from_env()):
    ...
    try:
        container.exec_run(['sh', '-c', 'echo ' + shlex.quote(source) +
                            ' > ' + settings['source']])
...
if __name__ == '__main__':
    print(judge(ruamel.yaml.YAML().load(open('settings.yaml'))
